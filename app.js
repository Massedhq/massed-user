...
let events = [];
    ...
    function saveEvent() {
      ...
      const ticketType = ticketTiers.map(t => t.type).join(', ');

      const onlineLink = type === 'Online' ? updateStreamLink() : '';
      const ev = { id: Date.now(), name, type, ticketType, tiers: [...ticketTiers], desc, date, time, location, onlineLink, qty, sold: 0, price: basePrice, salePrice, tags, coverUrl, galleryUrls, sendEmail, sendSMS, inPerson, active: true };
...
function nav(el,screen) {
  ...
  try {
    // Push state for back button support (skip in iframes to avoid SecurityError)
    ...
    document.querySelectorAll('.screen').forEach(function(s){if(s)s.classList.remove('active');});
